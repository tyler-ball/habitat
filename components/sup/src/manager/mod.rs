...
use crate::{
    ...
    event::{self, EventConnectionInfo, EventCore, EventStreamConfig},
...
use habitat_common::{outputln, types::ListenCtlAddr, FeatureFlag};
use habitat_core::{
    crypto::SymKey,
    env::{self, Config},
    fs::FS_ROOT_PATH,
...
use rustls::{
    internal::pemfile, AllowAnyAuthenticatedClient, NoClientAuth, RootCertStore, ServerConfig,
};
use serde_json;
use std::{
    self,
...
fn tls_config(config: &TLSConfig) -> Result<rustls::ServerConfig> {
    ...
    let cert_chain = pemfile::certs(cert_file)
        ...
        .map_err(|_| sup_error!(Error::InvalidCertFile(config.cert_path.clone())))?;

    let key = pemfile::rsa_private_keys(key_file)
        .and_then(|mut k| k.pop().ok_or(()))
        .map_err(|_| sup_error!(Error::InvalidKeyFile(config.key_path.clone())))?;
