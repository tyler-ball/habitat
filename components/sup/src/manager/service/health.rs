...
impl fmt::Display for HealthCheck {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            HealthCheck::Ok => "OK".fmt(f),
            HealthCheck::Warning => "WARNING".fmt(f),
            HealthCheck::Critical => "CRITICAL".fmt(f),
            HealthCheck::Unknown => "UNKNOWN".fmt(f),
        }
    }
}
         ...
         .and_then(move |check_result| {
             ...
             *service_health_result.lock()
             ...
             let interval = if check_result == HealthCheck::Ok {
                 ...
                 //
                 // TODO (CM): reconsider this logic; I believe the
                 // intention is to run more frequently in failure
                 // cases in order to more quickly recover a non-OK
                 // state. Nothing currently guarantees that default
             ...
             let instant =
                 Instant::now().checked_add(interval.into())
                               .expect("This should never happen with normal health check \
                                        interval sizes");
             // TODO (CM): add some validation to health check
             // intervals to constrain the size?
